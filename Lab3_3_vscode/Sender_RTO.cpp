...
// 计时线程所需
double RTO = 2;                 // 超时时间(s)
double sampleTime = 1;              // 实际时间(s)
double spEndTime,spStartTime;       // 用于统计实际的开始时间(s)
double estiTime=1;                  // 估算时间(s)
...
DWORD WINAPI TimeCheck(LPVOID lparam)
{
    while (true)
    {
        ...
        if (  lastTime / CLOCKS_PER_SEC >= RTO)
        {
            ...
            startTime = clock();
            spStartTime=clock();
            //cout<<"spStartTime is :"<<spStartTime<<endl;
            PreTickTime = false;
            timecount = 0;
            // 恢复线程并行
...
DWORD WINAPI RecvThread(LPVOID lparam)
{
    ...
    while (true)
    {
        ...
        if (!CheckSum&&checkID()&&checknewACK())   //Checksum应该是0,ID是应该是Sendbase
        {
            ...
            spEndTime=clock();
            //cout<<"spEndTime is : "<<spEndTime<<endl;
            sampleTime=(spEndTime-spStartTime)/CLOCKS_PER_SEC;
            //cout<<"sample time is: "<<sampleTime<<endl;
            estiTime = 0.875*estiTime + 0.125*sampleTime;
            RTO=2*estiTime;
