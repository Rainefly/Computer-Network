...
// 读写文件所需
string  File_Path = ".\\workfile3_2\\";
string  File_Des_Path = ".\\workfile_res3_2\\";
...
DWORD WINAPI TimeCheck(LPVOID lparam)
{
    while (true)
    {
        ...
        if (  lastTime / CLOCKS_PER_SEC == timeout)
        {
...
DWORD WINAPI RecvThread(LPVOID lparam)
{
    ...
    while (true)
    {
        ...
        if (!CheckSum&&checkID())   //Checksum应该是0,ID是应该是Sendbase
        {
            SendBase = getID(bufRecv) + 1;
            cout << "SendBase is : " << SendBase << " ID is: " << identifier << endl;
            PreTickTime = true;
            Sleep(10);
            ...
            {
                cout << "the Sendbase is not equal to ID sent, waiting for ACK. Now the Sendbase is:  " << SendBase << endl;
                PreTickTime = false;
...
int main()
{
    ...
    while (true) {
        ...
        endTime = clock();
        fileEndTime=clock();
        //cout <<   "Total time : " << ( endTime -  startTime) * 1.0 / 1000 << endl;
